...
# Make Parameters
MAKE_PARAMS="O=out ARCH=arm64 CC=$TC_DIR/bin/clang CLANG_TRIPLE=$TC_DIR/bin/aarch64-linux-gnu- LD=$TC_DIR/bin/ld.lld LLVM=1 LLVM_IAS=1 \ CROSS_COMPILE=$TC_DIR/bin/llvm-"

MAKE_PARAMS1="ARCH=arm64 CC=$TC_DIR/bin/clang CLANG_TRIPLE=$TC_DIR/bin/aarch64-linux-gnu- LD=$TC_DIR/bin/ld.lld LLVM=1 LLVM_IAS=1 \ CROSS_COMPILE=$TC_DIR/bin/llvm-"
...
dtb="$KERNEL_DIR/out/arch/arm64/boot/dts/vendor/qcom/yupik.dtb"
dtbo="$KERNEL_DIR/out/arch/arm64/boot/dts/vendor/qcom/lisa-sm7325-overlay.dtbo"
dtboimg="$KERNEL_DIR/out/arch/arm64/boot/dtbo.img"
...
build_kernel()
{
	...
	if [ $SILENCE = "1" ]
	then
		MAKE+=( $MAKE_PARAMS C-s )
	fi

	msger -n "|| Started Compilation ||"
	make -kj"$PROCS" $MAKE_PARAMS \
		V=$VERBOSE \
		"${MAKE[@]}" $MAKE_PARAMS 2>&1 | tee error.log
	if [ $MODULES = "1" ]
	then
	    msger -n "|| Started Compiling Modules ||"
	    make -j"$PROCS" $MAKE_PARAMS \
		 "${MAKE[@]}" modules_prepare
	    make -j"$PROCS" $MAKE_PARAMS \
		 "${MAKE[@]}" $MAKE_PARAMS1 modules INSTALL_MOD_PATH="$KERNEL_DIR"/out/modules
	    make -j"$PROCS" $MAKE_PARAMS \
		 "${MAKE[@]}" $MAKE_PARAMS1 modules_install INSTALL_MOD_PATH="$KERNEL_DIR"/out/modules
		...
		then
			...
			then
				...
				python3 "$KERNEL_DIR/scripts/dtc/libufdt/mkdtboimg.py" \
					create "$KERNEL_DIR/out/arch/arm64/boot/dtbo.img" --page_size=4096 "$KERNEL_DIR/$dtbo"
			fi
				gen_zip
			else
			if [ "$PTTG" = 1 ]
...
gen_zip()
{
	...
	mv $kernel $AK3_DIR/$kernel
	mv $dtb $AK3_DIR/dtb
	if [ $BUILD_DTBO = 1 ]
	then
		mv "$dtboimg" $AK3_DIR/dtbo.img
	fi
	cdir $AK3_DIR
